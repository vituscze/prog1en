...
# Suppose we have N boxes with different weights and we want to find the heaviest one.
# At our disposal, we have scales that can be used to compare weights of two boxes.
#
# a) How to find the heaviest box while using the scales as little as possible?
# b) Why is the previous result optimal? Show that we cannot do better.
# c) How to find the heaviest and lightest box while using the scales at most ceil(3N/2) - 2 times?
# d) [bonus] How to find the two heaviest boxes while using the scales at most N - 2 + ceil(log_2(N)) times?

# a) We pick the first box as the candidate for the heaviest one. Then we put the candidate and the
# second box on the scales. If the second box is heavier, we replace it as the candidate.
# We do this for all the remaining boxes. At the end, the candidate is the heaviest box and we used the
# scales N - 1 times.
#
# b) To see why this is optimal, consider what happens to the number of boxes that "lost" in at
# least one round. Each time we use the scales, this number increases by at most one. If we use
# the scales N - 2 (or fewer) times, we'll have at least 2 boxes that haven't lost a round and
# we can't tell which of these two is the maximum.
#
# c) For simplicity, assume N is even (showing it for odd N is very similar). First, we weigh the
# boxes in pairs (first with second, third with fourth, etc), which requires using the scales N/2 times.
# Notice that the heaviest box must be among the winners and the lightest one among the losers. We can
...
# is actually a power of 2, since 2**0 = 1).
...
total = 0
for n in range(1000): # Remember that range doesn't include the endpoint
    if n % 3 == 0 or n % 5 == 0:
        total = total + n
print(total)
